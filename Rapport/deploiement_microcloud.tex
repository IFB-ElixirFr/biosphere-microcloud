...
\begin{warningbox}
    Si les données ont été copiées depuis la prod et insérées telles quelles (sans \module{micPrestation}), il faut vérifier que le status des S\_id existants dans la table \DB{pkgdb.Sequenc}e soit 'inFunctional' et non 'inProduction' pour que les WF prennent bien en compte les nouveaux S\_id.
\end{warningbox}
