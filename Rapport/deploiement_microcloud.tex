...
Il faut avoir créé au préalable un utilisateur admin pour valider la prestation. En cela le script \script{create\_user.sh} peut servir.
...
\begin{mycolorbox}
	Une fois la prestation validée, il reste un problème : les données des banques nécessaires pour le script \textbf{loadGenomeNS.sh} ne sont pas stockées dans la VM permanente actuellement (données des banques gérées par CABRI).
