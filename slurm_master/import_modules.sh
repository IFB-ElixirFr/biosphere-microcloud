...
while IFS= read -r line; do
    ...
    if [ "${module_name}" = "bagsub" ]; then
        ...
        sed -i "s|BAGSUB_MPIRUN_OPTIONS='--mca orte_base_help_aggregate 0 --display-allocation'|BAGSUB_MPIRUN_OPTIONS='--allow-run-as-root --mca btl_tcp_if_exclude self,docker0,lo --mca orte_base_help_aggregate 0 --display-allocation'|" bagsub/linux-noarch/bin/bagsub
    fi
done < "modules.txt"
